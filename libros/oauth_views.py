...
def google_oauth_callback(request):
    ...
    try:
        ...
        return redirect(f'/oauth/login/?error={quote(f"Error con Google: {str(e)}")}')
...
def google_oauth_redirect(request):
    ...
    params = {
        'client_id': google_config["client_id"],
        'redirect_uri': getattr(settings, 'OAUTH_REDIRECT_URI', 'http://127.0.0.1:8000/api/auth/google/callback/'),  # Usar 127.0.0.1
        'scope': " ".join(scopes),
        'response_type': 'code',
        'access_type': 'offline',
        'prompt': 'consent',
